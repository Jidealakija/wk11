...
export default function page() {
  ...
  const list = filteredCourse.length > 0 ? filteredCourse.map(course => (
    ...
    <section key={course.id} className="flex flex-col items-center border-4 border-orange-500 p-4 rounded-xl">
      <Image src={course.image} alt={course.name} width={250} height={250}/>
      <h2><span className="font-bold">course name:</span>{course.name}</h2>
      <p><span className="font-bold">course duration:</span>{course.duration}</p>
    </section>
    </Link>
  ))  :(
  ...
  return (
    <div>
      <input type="text" name="text" id="text" placeholder="search for a course" className="text-black w-1/2 m-auto p-4 my-5 outline-none border-4 border-emerald-800 rounded-sm" value={item} onChange={onChange}/>
